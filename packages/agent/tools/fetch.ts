...
export const webFetchTool = tool({
  ...
  // approver, so a blanket "always require approval" would just wedge the
  // run with a dangling, never-approved tool call. Instead, unattended runs
  // are gated by HTTP method via externalWritePolicy: read-only GET/HEAD
  // auto-approve (the agent's tool allowlist *is* the pre-approval for those),
  // while mutating POST/PUT/PATCH/DELETE still require approval — an
  // unattended run has no human to approve a side-effecting external write,
  // so it must stay blocked rather than silently mutate a third party.
