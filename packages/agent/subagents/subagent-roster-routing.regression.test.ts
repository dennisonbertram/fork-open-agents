...
describe("Guard 1: plain gateway roster override preserves base directInference", () => {
  test("a roster entry with no directInference of its own routes through the base selection's direct-inference config", () => {
    resetCalls();
    ...
    // directAnthropicModel calls the provider with the direct-inference
    // config's OWN modelId (the real provider-side model name), not the
    // catalog id on the roster entry.
