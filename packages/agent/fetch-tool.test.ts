...
/**
 * Regression coverage for #1393 (DNS rebinding via curl `--resolve`) and
 * #1394 (unattended mutating HTTP methods must still require approval).
 *
...
async function needsApprovalResult(
  ...
  if (typeof needsApproval === "function") {
    return await Promise.resolve(
      needsApproval(
        input as never,
        executionOptions(experimental_context) as never,
      ),
    );
  }
...
describe("web_fetch unattended mutating-method approval (#1394)", () => {
  test("unattended GET auto-approves (no approval required)", async () => {
    const result = await needsApprovalResult(
      { url: "https://example.com", method: "GET" },
      { unattended: true },
    );
  ...
  test("unattended HEAD auto-approves (no approval required)", async () => {
    const result = await needsApprovalResult(
  ...
  test("unattended POST still requires approval", async () => {
    const result = await needsApprovalResult(
  ...
  test("unattended PUT/PATCH/DELETE still require approval", async () => {
  ...
  test("attended GET still requires approval (no auto-approve outside unattended)", async () => {
    const result = await needsApprovalResult(
      { url: "https://example.com", method: "GET" },
      {},
    );
