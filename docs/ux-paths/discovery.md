...
**Test-auth bypass** (`apps/web/lib/session/test-auth.ts`, the mechanism a curl-driven test phase should use): when `NODE_ENV=development` OR `OPEN_AGENTS_ENABLE_TEST_AUTH=1`, a request cookie `open_agents_test_user_id=dev-managed-runtime-user` resolves to a fixed fake session (`userId: dev-managed-runtime-user`) — checked *before* any real cookie/authorization-header presence check, so it works with zero prior state. The only route that **sets** this cookie is `GET /api/dev/managed-runtime-demo` (itself 404s unless `isTestAuthEnabled()`). Recommended flow for automated testing: start the server with `OPEN_AGENTS_ENABLE_TEST_AUTH=1`, call that endpoint once, capture the `Set-Cookie`, and replay it on all subsequent requests instead of driving real GitHub OAuth.
...
| GET | /api/dev/managed-runtime-demo | test-cookie | Dev-only: seed demo session + set test-auth cookie | query profileId? | demo payload, sets `open_agents_test_user_id` cookie \| 404 if disabled |
