...
| [research/testing-and-observability.md](research/testing-and-observability.md) | Local-vs-deployed test boundary on this stack (vitest-pool-workers vs wrangler dev vs deploy-only), per-primitive observability, trace correlation (none exists first-party — the ledger carries it), and the unattended-build credentials manifest |
| [research/e2e-harness-prior-art.md](research/e2e-harness-prior-art.md) | E2E tooling prior art: MCP conformance suites, the workers-oauth-provider conformance harness (near-direct lift), deterministic fake-brain precedent (testagent), DO eviction test helpers, OTel GenAI status; proposed M0 e2e suite |
