...
|---|-------|-------|-------------------------|
| 5 | Enable `OPEN_AGENTS_ENABLE_TEST_AUTH=1` on the custom **`dev`** environment only; delete any stale Preview branch var | Operator | 2026-08-20 22:52 local: flag added to Vercel `development` (local/`vercel dev` scope). Stable Dev URL is custom env `dev` and still HTML 404 on `dpl_4j7yWJeZCNtkPi5qGJjZUiNPL2C5`. Need `vercel env add … dev` then `vercel deploy --target=dev` from current `develop`. See [STORY-158 walk](../ux-paths/browser/walk-story-158.md) |
