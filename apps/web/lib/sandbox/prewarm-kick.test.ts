...
const spies = {
  ...
  claimSessionPrewarmRunId: mock(async (_sessionId: string, _runId: string) => true),
  clearSessionPrewarmRunId: mock(async (_sessionId: string, _runId: string) => undefined),
...
beforeEach(() => {
  ...
  spies.prewarmSessionSandbox.mockImplementation(async () => ({ status: "prewarmed" as const }));
});
