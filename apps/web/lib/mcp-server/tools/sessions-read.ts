...
export type McpSessionDetail = {
   ...
   * `toLastRunOutcome` in session-state.ts. Note: This field can briefly be
   * null while `activity` is "idle" immediately after a run finishes — this
   * is a transient window where the stream slot has cleared but the run
   * outcome has not yet been written. */
