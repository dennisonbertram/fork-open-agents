...
describe("source deletion write guards", () => {
  test("run status, context, and advance writes require a live source FK", () => {
    expect(functionBody("updateAgentLoopRunStatus", "updateAgentLoopRunContext"))
      .toContain("isNotNull(agentLoopRuns.loopId)");
    expect(functionBody("updateAgentLoopRunContext", "setInitialStepPointer"))
      .toContain("isNotNull(agentLoopRuns.loopId)");
    expect(functionBody("advanceRunToNextStep", "countStepRunsForNode"))
      .toContain("isNotNull(agentLoopRuns.loopId)");
    expect(
      functionBody("conditionallyTransitionRunStatus", "findStalledLoopRunCandidates"),
    ).toContain("isNotNull(agentLoopRuns.loopId)");
  });
  ...
  test("resume and retry reject a retained run with a typed source_deleted error", () => {
    expect(functionBody("resumeLoopRun", "retryCurrentStep")).toContain(
      'RunControlError("source_deleted"',
    );
    expect(functionBody("retryCurrentStep", "conditionallyTransitionRunStatus"))
      .toContain('RunControlError("source_deleted"');
  });
});
