...
describe("AgentSpecEditor", () => {
  ...
  // Opening tag of the segmented "Enabled" toggle button, isolated so we can
  // assert its active styling without matching the "Disabled" sibling.
  function enabledToggleTag(html: string): string {
    const labelIdx = html.indexOf(">Enabled<");
    const openIdx = html.lastIndexOf("<button", labelIdx);
    return html.slice(openIdx, labelIdx);
  }

  test("the Enabled toggle reads as active (green) when the agent is enabled", async () => {
    const { AgentSpecEditor } = await modulePromise;

    const html = renderToStaticMarkup(
      <AgentSpecEditor {...defaultEditorProps} initialEnabled={true} />,
    );

    // Active "on" state uses the codebase's emerald "enabled/good" treatment so
    // it's visually clear the agent is active (page feedback).
    expect(enabledToggleTag(html)).toContain("bg-emerald-600");
  });

  test("the Enabled toggle is not green when the agent is disabled", async () => {
    const { AgentSpecEditor } = await modulePromise;

    const html = renderToStaticMarkup(
      <AgentSpecEditor {...defaultEditorProps} initialEnabled={false} />,
    );

    expect(enabledToggleTag(html)).not.toContain("bg-emerald-600");
  });
});
