...
import { discardChangesRequestSchema } from "@/lib/git/http-schemas";
...
export async function POST(req: Request, context: RouteContext) {
  ...
  const parsed = discardChangesRequestSchema.safeParse(
  ...
  try {
    const result = await discardChanges({ sessionId, ...parsed.data });
